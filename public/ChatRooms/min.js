...
cht.addEventListener('click', ()=>{
    if(thisVal.value != ""){
        socket.emit('chat',{ user: thisName, msg: thisVal.value});
    }
});
...
socket.emit('message', {user: thisName, mess: ' join the chat!'});

window.addEventListener("beforeunload", ()=>{
    socket.emit('endchat', {user: thisName, mess: ' left the chat!'});
});
